...
- [ ] `POST /api/withdrawals/[id]/decision`: `{ action, reason? }` — pending-only; approve = balance re-check (decline "Insufficient balance" if short) → insert processed ledger `withdrawal` row → mark approved + decided_at/by; decline = mark declined + optional reason; ledger-insert failure leaves it pending
- [ ] Typecheck + lint clean
