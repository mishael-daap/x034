...
- `app/api/withdrawals/[id]/decision/route.ts` (new): `POST { action: "approve" | "decline", reason? }` (auth; admin gating deferred, dev-usable):
...
- `decided_by` is filled from the session but not enforced to any role yet — the endpoint is intentionally ungated until platform/admin auth lands (roadmap 5.1 defers it).
